...
export default function App() {
  ...
  async function refresh() {
    ...
    try {
      ...
      setTree({"":entries}); setExpanded(new Set([""])); setIndexed(paths);
      void refreshGit();
    } catch(e) { report(e); }
  }
  ...
  async function create(directory=false) {
    if (!root) return chooseFolder();
    const path=await ask(directory?"New folder":"New file",["Cancel","Create"],"Enter a path relative to this workspace. Its parent folder must already exist.","");
    if (!path) return;
    try { await invoke("create_entry",{path,directory});await refresh();if(!directory)await openFile(path);setStatus("Created "+path); } catch(e) {report(e);}
  }
  ...
  function treeItems(parent="",depth=0): React.ReactNode {
    return (tree[parent]||[]).map(entry=><div key={entry.path}>
      <button className={"tree-row "+(active===entry.path?"active":"")} style={{paddingLeft:12+depth*14}} title={entry.path} onClick={()=>entry.directory?toggleFolder(entry.path):openFile(entry.path)}>
      ...
      </button>{entry.directory&&expanded.has(entry.path)&&treeItems(entry.path,depth+1)}
    </div>);
  }
    ...
    <div className="workbench">
      ...
      {sidebar&&<><aside className="sidebar" style={{width:sidebarWidth}}>
        ...
        {panel==="files"&&<>
          ...
          <div className="tree">{root?treeItems():<div className="sidebar-empty"><FolderOpen size={28}/><p>Your files, together.</p><small>Open a folder to explore your project.</small><button className="primary" onClick={chooseFolder}>Open folder</button></div>}</div>
      ...
      <section className="content">
        ...
        <div className="editing-area">
          {diff!==null?<div className="diff-view"><div><GitBranch size={16}/><b>Changes · {active}</b><button onClick={()=>setDiff(null)}><X size={16}/></button></div><pre>{diff||"No changes against HEAD. Untracked files have no HEAD diff."}</pre></div>:file?<div className="editor-splits">{[0,...(split?[1]:[])].map(index=><div className="monaco-pane" key={index}><Editor path={modelUri} keepCurrentModel language={languageFor(file.path)} value={file.text} theme={preferences.light?"vs":"veyra"} onMount={mounted} onValidate={()=>setProblems(monaco.editor.getModelMarkers({}))} onChange={text=>setTabs(all=>all.map(t=>t.path===file.path?{...t,text:text??""}:t))} options={editorOptions}/></div>)}</div>:<div className="welcome">
            <div className="welcome-top"><span className="eyebrow">A LITTLE SPACE. A LOT OF POSSIBILITY.</span><span className="version">Veyra · 0.2</span></div>
