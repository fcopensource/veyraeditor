> **Explorer preview (September 23):** This separate build is based on the recovered source checkout, not the missing 0.5 source. It intentionally has its own application name and bundle identifier, so it does not replace Veyra 0.5 or share that app's settings. AI Studio and other later features are not included in this recovered baseline.
>
> **Nested creation:** Click a folder in the Explorer; the **Create in** bar shows the target. Use New file / New folder in the toolbar or the buttons beside a folder. Enter a name such as `Button.tsx`, or a nested path such as `components/ui/Button.tsx`. Missing parent folders are created automatically, the tree expands to the new entry, and duplicate names stay in the dialog so you can correct them. The folder icon in **Create in** resets the destination to the project root. Refresh preserves expanded folders. Escape cancels the creation dialog.
